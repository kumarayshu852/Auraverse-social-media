...
//find user using usernmae ,email,location,name
...
export const unfollowUser = async (req, res) => {
    try {
       ...
       res.json({sucess:true,message:"you are no longer following this user"})
...
export const sendConnectionRequest =async(req,res)=>{
    try{
        ...
        const connectionRequests =await Connection.find({from_user_id: user_id, created_at:{$gt:last24Hours}})
        if(connectionRequests.length>=20){
            return res.json({success:false,message:"You have send more than 20 connection requests in the last 24 hours"})
        }

        // chech if user are already conncted
...
export const acceptConnectionRequest =async(req,res)=>{
    try{
        ...
        const connection =await Connection.findOne({from_user_id, to_user_id: userId})
